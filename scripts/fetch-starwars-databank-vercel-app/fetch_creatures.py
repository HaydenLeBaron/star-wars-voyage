...
def fetch_page(page, limit=100):
    url = f"https://starwars-databank-server.onrender.com/api/v1/creatures?page={page}&limit={limit}"
...
def main():
    ...
    while True:
        ...
        all_creatures.extend(data)
        total_expected = info.get("total", "unknown")
        print(f"Page {page} fetched: got {len(data)} creatures. Total so far: {len(all_creatures)} / {total_expected}")
    ...
    output_path = os.path.join(script_dir, "star-wars-raw-context", "star-wars-databank-vercel-app", "star-wars-creatures-data.json")
    ...
    print(f"\nSuccessfully saved {len(all_creatures)} creatures to {output_path}")
