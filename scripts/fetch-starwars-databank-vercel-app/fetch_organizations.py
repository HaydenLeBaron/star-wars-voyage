...
def fetch_page(page, limit=100):
    url = f"https://starwars-databank-server.onrender.com/api/v1/organizations?page={page}&limit={limit}"
...
def main():
    ...
    while True:
        ...
        print(f"Page {page} fetched: got {len(data)} organizations. Total so far: {len(all_organizations)} / {total_expected}")
    ...
    output_path = os.path.join(script_dir, "star-wars-raw-context", "star-wars-databank-vercel-app", "star-wars-organizations-data.json")
    ...
    print(f"\nSuccessfully saved {len(all_organizations)} organizations to {output_path}")
