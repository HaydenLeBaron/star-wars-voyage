...
def fetch_page(page, limit=100):
    url = f"https://starwars-databank-server.onrender.com/api/v1/characters?page={page}&limit={limit}"
...
def main():
    ...
    while True:
        ...
        print(f"Page {page} fetched: got {len(data)} characters. Total so far: {len(all_characters)} / {total_expected}")
    ...
    output_path = "star-wars-character-data.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(all_characters, f, indent=2, ensure_ascii=False)
        
    print(f"\nSuccessfully saved {len(all_characters)} characters to {output_path}")
