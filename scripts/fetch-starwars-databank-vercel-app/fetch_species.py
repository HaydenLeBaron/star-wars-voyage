...
def fetch_page(page, limit=100):
    url = f"https://starwars-databank-server.onrender.com/api/v1/species?page={page}&limit={limit}"
...
def main():
    ...
    while True:
        ...
        print(f"Page {page} fetched: got {len(data)} species. Total so far: {len(all_species)} / {total_expected}")
    ...
    output_path = os.path.join(script_dir, "star-wars-raw-context", "star-wars-databank-vercel-app", "star-wars-species-data.json")
    ...
    print(f"\nSuccessfully saved {len(all_species)} species to {output_path}")
