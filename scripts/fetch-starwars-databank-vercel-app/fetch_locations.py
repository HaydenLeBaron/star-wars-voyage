...
def fetch_page(page, limit=100):
    url = f"https://starwars-databank-server.onrender.com/api/v1/locations?page={page}&limit={limit}"
...
def main():
    ...
    while True:
        ...
        print(f"Page {page} fetched: got {len(data)} locations. Total so far: {len(all_locations)} / {total_expected}")
    ...
    output_path = os.path.join(script_dir, "star-wars-raw-context", "star-wars-databank-vercel-app", "star-wars-locations-data.json")
    ...
    print(f"\nSuccessfully saved {len(all_locations)} locations to {output_path}")
